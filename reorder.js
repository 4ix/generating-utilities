...
var totalIndex = 10000
var selectingNumber = 10000
...
//중복 없이 번호 랜덤 생성
    ...
    for (i=0; i<selectingNumber; i++) {   //check if there is any duplicate index
      ...
      if (randomIndexArray.indexOf(randomNum) === -1) {
        ...
        const metadata = {
          name: `Chris Brown dives into NFT #${randomIndexArray[i]}`,
        ...
        // 새 폴더로 이동하면서 파일명 변경
        fs.rename(`./images/${i+1}.png`, `./images/rename/${randomIndexArray[i]}.png`, () => {
        });
        
        // 메타데이터 새로 생성
        const metadataJSON = JSON.stringify(metadata);
        fs.writeFileSync(`./metadatas/${randomIndexArray[i]}.json`, metadataJSON)
        console.log(`바뀐번호: ${randomIndexArray[i]} from 원래번호: ${i+1}`)
